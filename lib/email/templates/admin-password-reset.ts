...
export const adminPasswordResetTemplate = (data: {
  resetUrl: string;
  expiresInMinutes: number;
}) => {
  const content = `
    <p style="margin: 0 0 10px; font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: #9ca3af; font-weight: 700; font-family: -apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif;">Admin security</p>
    <h2 style="margin: 0 0 20px;">Reset your admin password</h2>

    <p>
      We received a request to reset your admin password. Use the button below to continue.
      This link expires in <strong>${data.expiresInMinutes} minutes</strong> and can only be used once.
    </p>

    <a href="${data.resetUrl}" class="button" style="margin-top: 24px;">Reset Admin Password</a>
    ...
    <div class="info-box" style="margin-top: 24px;">
      ...
      <p style="margin: 0; font-size: 13px; color: #6b7280; line-height: 1.7; font-family: -apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif;">
        You can safely ignore this email. Your current password remains unchanged.
      </p>
    </div>
  `;
